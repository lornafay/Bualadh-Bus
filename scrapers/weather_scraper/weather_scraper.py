...
def post_weather_to_table(json):
    ...
    wind_dir = f"{json['properties']['timeseries'][0]['data']['instant']['details']['wind_from_direction']}"
    ...
    sql = f"INSERT INTO current_weather (`time`, `temperature`, `wind_speed`, `wind_dir`, `clouds`, `rain`, " \
        f"`pressure`, `humidity`, `dew_point`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);"

    # prepare entries
    items = [time, current_temp, wind_speed, wind_dir, clouds, rain, pressure, humidity, dew_point]
    ...
    def weather_forecast(substring):
        for i in range(len(json['properties']['timeseries'])):
            ...
            if substring in fullstring:
                ...
                # prepare sql statement
                sql_forecast = f"INSERT INTO weather_forecast (`time`, `rain`, `temp`, `dew_point`, `humidity`, `pressure`) VALUES (%s, %s, %s, %s, %s, %s);"
